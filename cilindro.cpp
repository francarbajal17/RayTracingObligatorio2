...
cilindro::cilindro(vec3 centro, double radio, double altura, vec3 direccion,
	color ambient, color especular, color difuso, double indice_reflexion, double indice_refraccion, double indice_especular, double indice_transparencia) {
	this->centro = centro;
	this->radio = radio;
	this->altura = altura;
...
hitRecord* cilindro::intersects(const rayo& ray) {
    float tCaps;
    vec3 normalCaps;
    bool hitCaps = intersectsCaps(ray.getOrigin(), ray.getDirection(), tCaps, normalCaps);

    float tSide;
    bool hitSide = intersectsSide(ray.getOrigin(), ray.getDirection(), tSide);
    ...
    if (hitCaps && hitSide) {
        ...
        closestHit->position = ray.getOrigin() + ray.getDirection() * t0;
...
bool cilindro::intersectsSide(const vec3& rayOrigin, const vec3& rayDirection, float& t) const {
    ...
    for (double t_val : {t1, t2}) {
        if (t_val < 0) continue; // Intersection is behind the ray's origin
